...
- Units in a group are listed **smallest to largest**, with factors relative to the group's base
  unit, the one with factor 1.

Add a new group at the end: a group's index is what `lookupUnit` returns, and ports read it from
`data/units.json`.
...
npm run build          # tsc twice: dist/esm and dist/cjs, then data/*.json
...
### Test vectors and the JSON data

Most behaviour is pinned in [`test/vectors.json`](test/vectors.json): one list of cases per public
function, run against the TypeScript by `test/vectors.test.ts`. The same file ships in the package
as `data/test-vectors.json`, so ports to other languages are checked against it too. Add a case
there rather than an `expect` in a test file, unless the check is about the tables themselves.
Write expected values by hand, and keep them language-neutral:
[docs/data-format.md](docs/data-format.md) says how to write `undefined`, NaN and optional
arguments. A new public function needs vectors; a test fails until it has them.

`data/units.json` is generated by `scripts/export-data.mjs` from the built code. Don't edit it:
change `src/`, and `test/data.test.ts` checks that the export still carries everything.
